...
class AppStore extends Component {
  ...
  render() {
    ...
    let filteredAppList = appsList.filter(
      eachItem => eachItem.category === selectedTab,
    )

    if (searchText.length !== 0) {
      filteredAppList = filteredAppList.filter(eachItem =>
        eachItem.appName.toLowerCase().includes(searchText.toLowerCase()),
      )
    }
      ...
      <div className="outer-cont">
        ...
        <div className="tab-cont">
          {tabsList.map(eachItem => (
            <TabItem
              itemValue={eachItem}
              tabChnage={this.onTabChange}
              isActice={selectedTab === eachItem.tabId}
            />
          ))}
        </div>
        <div className="app-cont">
          {filteredAppList.map(eachItem => (
            <AppItem itemValue={eachItem} />
          ))}
        </div>
      </div>
    )
